  ...
  int Init_System( reax_system *system, control_params *control, char *msg )
  {
    ...
    if( control->hbond_cut > 0 )
    {
      for( i = 0; i < system->n; ++i ) 
      {
        ...
        if (system->reax_param.sbp[ system->packed_atoms[i].type ].p_hbond == 1 && system->packed_atoms[i].type >= 0)
        {
