                       ...
                       storage *workspace, MPI_Comm comm )
{
  ...
  system->Hindex = (int *)scalloc(system->total_cap, sizeof(int), "Hindex", comm);
  system->num_bonds   = (int *)scalloc(system->total_cap, sizeof(int), "num_bonds", comm);
  system->num_hbonds  = (int *)scalloc(system->total_cap, sizeof(int), "num_hbonds", comm);
                     ...
                     char *msg )
{
  ...
  system->Hindex      = (int *)realloc(system->Hindex, total_cap * sizeof(int));
  system->num_bonds   = (int *)realloc(system->num_bonds , total_cap * sizeof(int));
  system->num_hbonds  = (int *)realloc(system->num_hbonds, total_cap * sizeof(int));
  return SUCCESS;
}
...
void DeAllocate_System( reax_system *system )
{
  ...
  sfree( system->packed_atoms, "system->my_packed");
  sfree( system->Hindex, "system->Hindex");
  sfree( system->num_bonds , "system->num_bonds");
  sfree( system->num_hbonds, "system->num_hbonds");
...
void DeAllocate_Workspace( control_params *control, storage *workspace )
{
  ...
  sfree( workspace->bo_dboc, "bo_dboc" );//total_bond_order+Delta_boc;
  ...
  sfree( workspace->fCdDelta, "fCdDelta" );
                        ...
                        MPI_Comm comm, char *msg )
{
  ...
  workspace->bo_dboc = (rvec2*) smalloc( total_real * 2, "bo_dboc", comm );
  ...
  workspace->fCdDelta = (rvec4*) scalloc( total_cap, sizeof(rvec4), "fCdDelta", comm );
...
static void Reallocate_Neighbor_Full_List(reax_list *far_nbrs_full, int n,
                                          int num_intrs, MPI_Comm comm){
  Delete_List( far_nbrs_full, comm );
  if(!Make_List( n, num_intrs, TYP_FAR_NEIGHBOR_FULL, far_nbrs_full, comm )){
    fprintf(stderr, "Problem in initializing far nbrs list. Terminating!\n");
    MPI_Abort( comm, INSUFFICIENT_MEMORY );
  }   
}
                                   ...
                                   MPI_Comm comm )
{
  ...
  for( i = 0; i < system->n; ++i )
    ...
    if( (id = system->Hindex[i]) >= 0 ) 
    {
      //total_hbonds += system->my_atoms[i].num_hbonds;
      total_hbonds += system->num_hbonds[i];
    }
                                  ...
                                  MPI_Comm comm )
{
  ...
  *est_3body = 0;
  for( i = 0; i < system->N; ++i )
  {
    ...
    *est_3body += SQR(system->num_bonds[i]);
    *total_bonds +=   system->num_bonds[i];
