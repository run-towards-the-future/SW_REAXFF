...
void NPairFullBinAtomonlySunway::build(NeighList *list)
{
  ...
  //int ibins; 
  //for(ibins = 0; ibins < mbins; ibins++)
  //{
  //  for(k = 0; k < nstencil; k++)
  //  {
  //    if(ibins + stencil[k] >= 0 && ibins + stencil[k] < mbins)
  //    {
  //      int js = binpackhead[ibins + stencil[k]];
  //      int je = binpackhead[ibins + stencil[k] + 1];
  //      for(j = js; j < je; j++)
  //      {
  //        jtype = binpack[j].type; 
  //        for(i = binpackhead[ibins]; i < binpackhead[ibins+1]; i++)
  //        {
  //          if(binpack[i].id != binpack[j].id && binpack[i].id < nlocal)
  //          {
  //            delx = binpack[i].x[0] - binpack[j].x[0];
  //            dely = binpack[i].x[1] - binpack[j].x[1];
  //            delz = binpack[i].x[2] - binpack[j].x[2];
  //            rsq = delx*delx + dely*dely + delz*delz;
  //            itype = binpack[i].type; 
  //            if(rsq <= cutneighsq[itype][jtype])  
  //            { 
  //              //firstneigh[i][numneigh[i]++] = binpack[j].id;
  //              firstneigh[binpack[i].id][numneigh[binpack[i].id]++] = binpack[j].id;
  //            }
  //          }
  //        }//for-i
  //      }//for-j
