...
FixStyle(qeq/reax/sunway,FixQEqReaxSunway)

#else

#ifndef LMP_FIX_QEQ_REAX_SUNWAY_H
#define LMP_FIX_QEQ_REAX_SUNWAY_H

#include "fix.h"

#define PACK_SPARSE_MATVEC 11235813
namespace LAMMPS_NS {

class FixQEqReaxSunway : public Fix {
 public:
  FixQEqReaxSunway(class LAMMPS *, int, char **);
  ~FixQEqReaxSunway();
  int setmask();
 ...
 protected:
  ...
  virtual void compute_H_Full_Newtoff();
  double calculate_H(double,double);
  virtual void calculate_Q();

  virtual int CG(double*,double*);
  virtual int CG_new(double*,double*);
  virtual int CG_v2(double*,double*);
  //int GMRES(double*,double*);
  virtual void sparse_matvec(sparse_matrix*,double*,double*);
  virtual void sparse_matvec_noff(sparse_matrix*,double*,double*);
  double *sparse_x, *sparse_b;
  virtual void sparse_matvec_comm(sparse_matrix*, double *, double *);
