...
void Reset_Atoms( reax_system* system, control_params *control )
{
  ...
  if( control->hbond_cut > 0 )
    for( i = 0; i < system->n; ++i ) {
      ...
      if( system->reax_param.sbp[system->packed_atoms[i].type ].p_hbond == 1 )
      {
        //atom->Hindex = system->numH++;
        system->Hindex[i] = system->numH++;
      }
      else 
      {
        //atom->Hindex = -1;
...
void Reset_Workspace( reax_system *system, storage *workspace )
{
  ...
  memset( workspace->bo_dboc, 0, system->total_cap * sizeof( double )*2 );
  memset( workspace->dDeltap_self, 0, system->total_cap * sizeof( rvec ) );
  memset( workspace->fCdDelta, 0, system->total_cap * sizeof(rvec4) );
