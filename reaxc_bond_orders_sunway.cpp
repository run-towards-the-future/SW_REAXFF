                              ...
                              storage *workspace, reax_list **lists )
{
  ...
  bond_order_data *bo_ij, *bo_ji, *bo_t;
         ...
         storage *workspace, reax_list **lists, output_controls *out_control )
{
  ...
  GPTLstart("reaxc bond orders c");
  BO_C(&param);
  GPTLstop("reaxc bond orders c");
  ...
  //{
  //  workspace->total_bond_order[i] = workspace->bo_dboc[i][0];
  //}
 ...
 /* Calculate Deltaprime, Deltaprime_boc values */
  ...
  //for(i = 0; i < system->total_cap; i++)
  //{
  //  workspace->bo_dboc[i][0] = workspace->total_bond_order[i];
  //}
  //printf("addresses: dboc %p %p, tbo %p %p", workspace->bo_dboc, workspace->bo_dboc + system->total_cap,
  //workspace->total_bond_order, workspace->total_bond_order + system->total_cap);
  ...
  //for(i = 0; i < system->total_cap; i++)
  //{
  //  if (workspace->bo_dboc[i][0] != workspace->total_bond_order[i]){
  //      printf("bo_dboc and total_bond_order differ: %d %.10g %.10g\n", i, workspace->bo_dboc[i][0], workspace->total_bond_order[i]);
  //  }
  //  workspace->bo_dboc[i][0] = workspace->total_bond_order[i];
  //}
  ...
  for( j = 0; j < system->N; ++j )
  {
    ...
    if (type_j < 0) 
    {
      ...
      //workspace->bo_dboc[j][0] = workspace->total_bond_order[j];
      continue;
    }
  ...
  //for(j = 0; j < system->total_cap; j++)
  //{
  //  workspace->Delta_boc[j] = workspace->bo_dboc[j][1];
  //}

  //for(i = 0; i < bonds->num_intrs; i++)
  //{
  //  bonds->select.bond_list[i].bo_data = bo_data_list[i];
  //}
