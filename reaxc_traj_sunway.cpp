                     ...
                     output_controls *out_control, mpi_datatypes *mpi_data )
{
  ...
  for( i = 0; i < system->n; ++i ) {
    ...
    sprintf( out_control->line, INIT_DESC,
             system->packed_atoms[i].orig_id, system->packed_atoms[i].type, p_atom->name,
             system->reax_param.sbp[ system->packed_atoms[i].type].mass );
                ...
                output_controls *out_control, mpi_datatypes *mpi_data)
{
  ...
  my_bonds = 0;
  bond_order_data *bo_data_ij;
  for( i=0; i < system->n; ++i ) {
    for( pj = Start_Index(i, bonds); pj < End_Index(i, bonds); ++pj ) {
      bo_ij = &( bonds->select.bond_list[pj] );
      bo_data_ij = &( bonds->bo_data_list[pj]);
      j = bo_ij->nbr;
      ...
      if( system->packed_atoms[i].orig_id <= system->packed_atoms[j].orig_id &&
        ...
        case OPT_BOND_BASIC:
          sprintf( out_control->line, BOND_BASIC,
                   system->packed_atoms[i].orig_id, system->packed_atoms[j].orig_id,
                   bo_ij->d, BO_list[pj]);
        ...
        case OPT_BOND_FULL:
          ...
          sprintf(out_control->line, BOND_FULL,
                  system->packed_atoms[i].orig_id, system->packed_atoms[j].orig_id,
                  bo_ij->d, BO_list[pj], bo_data_ij->BO_s,
                  ...
                  output_controls *out_control, mpi_datatypes *mpi_data )
{
  ...
  for( j = 0; j < system->n; ++j )
    for( pi = Start_Index(j, bonds); pi < End_Index(j, bonds); ++pi ) {
      ...
      if(BO_list[pi] >= control->bg_cut ) // physical j&i bond
        for( pk = Start_Index( pi, thb_intrs );
          ...
          if( system->packed_atoms[i].orig_id < system->packed_atoms[k].orig_id &&
              BO_list[angle_ijk->pthb] >= control->bg_cut ) // physical j&k bond
        ...
        for( pk = Start_Index( pi, thb_intrs );
          ...
          if( system->packed_atoms[i].orig_id < system->packed_atoms[k].orig_id &&
            ...
            sprintf( out_control->line, ANGLE_BASIC,
                     system->packed_atoms[i].orig_id, system->packed_atoms[j].orig_id,
                     system->packed_atoms[k].orig_id, RAD2DEG( angle_ijk->theta ) );
