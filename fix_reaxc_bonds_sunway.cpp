...
FixReaxCBondsSunway::FixReaxCBondsSunway(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
FixReaxCBondsSunway::~FixReaxCBondsSunway()
{
  MPI_Comm_rank(world,&me);
...
int FixReaxCBondsSunway::setmask()
{
  int mask = 0;
  mask |= END_OF_STEP;
  return mask;
...
void FixReaxCBondsSunway::setup(int vflag)
{
  end_of_step();
}
...
void FixReaxCBondsSunway::end_of_step()
{
...
void FixReaxCBondsSunway::Output_ReaxC_Bonds(bigint ntimestep, FILE *fp)
...
void FixReaxCBondsSunway::FindBond(struct _reax_list *lists, int &numbonds)
{
  int *ilist, i, ii, inum;
  int j, pj, nj;
  tagint jtag;
  double bo_tmp,bo_cut;
  double *BO_list = reaxc->lists->BO_list;
...
void FixReaxCBondsSunway::PassBuffer(double *buf, int &nbuf_local)
{
  int i, j, k, numbonds;
  int nlocal = atom->nlocal;
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    buf[j+1] = reaxc->workspace->bo_dboc[i][0];
...
void FixReaxCBondsSunway::RecvBuffer(double *buf, int nbuf, int nbuf_local,
...
int FixReaxCBondsSunway::nint(const double &r)
{
  int i = 0;
...
void FixReaxCBondsSunway::destroy()
{
  memory->destroy(abo);
...
void FixReaxCBondsSunway::allocate()
{
...
double FixReaxCBondsSunway::memory_usage()
{
  double bytes;
